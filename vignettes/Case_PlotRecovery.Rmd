...
ofolder <- '../inst/extdata/'# 'C:\\Users\\keers001\\OneDrive - WageningenUR\\RETURN\\Data\\RETURN\\20200107_Upscaling\\CaseStudy\\Data\\'
# base name of the recovery indicator files
basename <- c('Stab_monthly_maxBreak_F_obspyr_12_inp_segmented_shortDenseTS_TRUE_nPre_2_nDist_12_nPostMin_4_nPostMax_6_h_15_2_2')
# directory where the figures will be stored
figfolder <- '../inst/extdata/'# 'C:\\Users\\keers001\\OneDrive - WageningenUR\\RETURN\\Data\\RETURN\\20200107_Upscaling\\CaseStudy\\Figures\\'
...
stab <- stack(file.path(ofolder, paste0( basename,'.gri')))
```

# Generate point coordinates of the site centroids
```{r}
# generate point coordinates of site centroids
(sites <- data.frame(longitude = c( mean(extent(stab)[1:2])), latitude = c(mean(extent(stab)[3:4]))))
...
sites <- st_transform(sites, crs(world))
...
dtsbr <- seq(min(dts), max(dts), by = "1 month")#"1 day"
  ...
  pointsize = 15)
    ...
    for (ii in 1:5){
      plot(zoo(tsii[ii,],dtsst), ylab = 'NBR', type = 'o')
